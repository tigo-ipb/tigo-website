...
export default function LegendRow({ label, value, percent, color }) {
    return (
        <div className={`flex items-center justify-between p-3`}>
            ...
            <div className={`text-lg font-semibold p-2.5 rounded-[8px] ${color === 'bg-sky-500' ? 'bg-sky-100 text-sky-500' : color === 'bg-green-500' ? 'bg-green-100 text-green-500' : color === 'bg-yellow-500' ? 'bg-yellow-100 text-yellow-500' : 'bg-red-200 text-red-500'}`}>
                {percent}%
            </div>
        </div>
    );
