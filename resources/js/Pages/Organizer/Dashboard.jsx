...
export default function Dashboard({ auth, stats, topEvents, recentBookings, currentEvent }) {
    
    // Total keseluruhan tiket untuk di tengah Donut
    const totalTickets = stats.total_tickets_sold + stats.total_tickets_available;
    
    // Kalkulasi Persentase Donut
    ...
    // --- Konfigurasi ApexCharts: Donut Chart (Penjualan Tiket) ---
    const donutOptions = {
                    ...
                    labels: {
                        ...
                        total: { 
                            ...
                            formatter: function (w) {
                                return new Intl.NumberFormat('id-ID').format(totalTickets); 
                            }
                        }
                    }
                }
    ...
    // --- Konfigurasi ApexCharts: Area Chart (Pendapatan Penjualan) ---
    const areaOptions = {
        ...
        yaxis: { 
            labels: { 
                formatter: (value) => {
                    // Merubah format agar tampil "10jt", "500k", dll
    ...
    // Data array bulanan dari Controller
    const areaSeries = [{
        name: 'Aktual',
        ...
        <DashboardLayout>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button className="bg-blue-500 text-white text-xs font-medium px-3 py-1.5 rounded-full flex items-center gap-1">
                                    Pendapatan <IconChevronDown size={14} />
                                </button>
                            </div>
                                ...
                                {topEvents.length > 0 ? topEvents.map((event, i) => (
                                    <div key={i} className="flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            <div className="w-6 h-6 rounded-full bg-blue-50 flex items-center justify-center text-blue-500 font-bold text-xs shrink-0">
                                                {i + 1}
                                            </div>
                                            <p className="text-sm font-medium text-gray-700 truncate w-32 md:w-40">{event.name}</p>
                                        </div>
                                        <p className="text-sm font-bold text-green-500">{formatRupiah(event.revenue)}</p>
                                    </div>
                                )) : (
                    ...
                    {/* Baris 3: Area Chart (Pendapatan Penjualan) */}
                    <div className="bg-white p-6 rounded-2xl border border-gray-100">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button className="bg-blue-500 text-white text-xs font-medium px-3 py-1.5 rounded-full flex items-center gap-1">
                                    Tahun ini <IconChevronDown size={14} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-5">
                            {recentBookings.length > 0 ? recentBookings.map((booking, i) => (
                                <div key={i} className="flex gap-4">
                                    <div className="w-10 h-10 shrink-0 rounded-xl bg-blue-50 text-blue-500 flex items-center justify-center">
                                        <IconTicket size={20} />
                                    </div>
                                    <div className="text-xs leading-relaxed">
                                        <p className="text-gray-700">
                                            <span className="font-bold">{booking.buyer_name}</span> membeli tiket <span className="font-bold text-blue-500">{booking.event_name}</span>
                                        </p>
                                        <p className="text-gray-400 mt-0.5">{booking.time_ago}</p>
                                    </div>
                                </div>
                            )) : (
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                            ...
                            <div className="relative w-full md:w-80">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-100"
                                />
                            </div>
                        </div>
                                    ...
                                    {recentBookings.length > 0 ? recentBookings.map((booking, i) => (
                                        <tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                                            ...
                                            <td className="py-4 px-4 text-xs text-gray-600">
                                                {booking.date}<br/><span className="text-gray-400">{booking.time}</span>
                                            </td>
